...
void SoftmaxWithWeightLossLayer<Dtype>::Reshape(
  ...
  inner_num_ = bottom[0]->count(softmax_axis_ + 1);
   // add the weight supported
  int weight_size = this->layer_param_.softmaxweight_param().labelweight_size();
  int dim = prob_.count()/outer_num_;
  labelweight->Reshape(1, dim, 1, 1);
  float* weight = labelweight->mutable_cpu_data();
  if ( weight_size > 0 )
  {
     CHECK_EQ(weight_size, dim) <<
         "weight must has the same size with channels.";
	 // weight here is weight, not probability.
     for(int i = 0; i < dim; i++) 
