...
/* // do not need decodedatum, just for database
...
*/
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    datum.ParseFromString(cursor_->value());
/*
    // ignore the encoded flag, include it in the future
    cv::Mat cv_img;
    if (datum.encoded()) {
      if (force_color) {
...
*/
    if (this->output_labels_) {
        ...
        for(int label_index=0; label_index<datum.label_size(); label_index++)
        {
            top_label[item_id*datum.label_size() + label_index] = datum.label(label_index);
        }
    }
